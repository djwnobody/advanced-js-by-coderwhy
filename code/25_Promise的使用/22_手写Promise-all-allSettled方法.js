    ...
    const resolve = (value) => {
      if (this.status === PROMISE_STATUS_PENDING) {
        ...
        queueMicrotask(() => {
          ...
          this.value = value
          // 为了处理两个独立的then；两个独立的then意味着要执行两个成功回调,所以放数组里
...
// 进入allSettled(),128行,遍历[p1, p2, p3],给p1身上添加then的两个回调,但不执行。注意：then方法只是给promise对象身上添加回调,真正的执行在resolve内部。
// 2.then(res => { console.log(res)})
// 给HYPromise.allSettled身上添加一个回调
// 3.147行执行p1的宏任务,resolve(111),进入32行,修改p1的状态为fulfilled,执行p1身上的成功回调,也就是130行
// 4.执行到p3的成功回调,进入133行,修改allSettled的状态,执行allSettled的成功回调,即161行
